...
def run(disp):
    ...
    # Change SSID / PASSWORD below as desired.
    # WiFi — STA mode only, do not touch AP_IF (driver is broken on this build)
        ...
        while sent < total:
            ...
            while retries < 40:
                try:
                    ...
                    if e.args[0] in (105, 103, 118):  # ENOBUFS, ECONNABORTED, EHOSTUNREACH
                        time.sleep_ms(50)
                        retries += 1
                    else:
                        raise
    ...
    def handle(conn):
        ...
        try:
            ...
            if method == 'GET':
                ...
                if path in ('/', '/browse'):
                    ...
                    try:
                        ...
                        if is_text:
                            # Stream the pre block in CHUNK-sized pieces to avoid
                            # building a single large escaped string in RAM.
                            def gen_view():
                                if sz > MAX_VIEW:
                                    yield '<p class="w">Showing first {} KB of {} KB.</p>'.format(
                                        MAX_VIEW//1024, sz//1024)
                                yield '<pre>'
                                text = raw_data.decode('utf-8', 'replace')
                                start = 0
                                while start < len(text):
                                    yield htmlesc(text[start:start + CHUNK])
                                    start += CHUNK
                                yield '</pre>'
                                yield '<a class="b" href="/dl?p={}">Download</a>'.format(urlenc(fp))
                            stream_page(conn, 'View: ' + htmlesc(fname), gen_view())
                        else:
                            stream_page(conn, 'View: ' + htmlesc(fname), [
                                '<p class="w">Binary file ({} bytes) — download to inspect.</p>'.format(sz),
                                '<a class="b" href="/dl?p={}">Download</a>'.format(urlenc(fp))
                            ])
                    except Exception as e:
                        stream_page(conn, 'View: ' + htmlesc(fname), [
                            '<p class="er">Read error: {}</p>'.format(htmlesc(str(e))),
                            '<a class="b" href="/dl?p={}">Download</a>'.format(urlenc(fp))
                        ])
            ...
            stat_bar('OSError {}: {}'.format(e.args[0], str(e)[:20]), RED)
            time.sleep_ms(2000)
        except Exception as e:
